...
void setupUltrasonicSensor() {
  ...
  sendBTMessage("Ultrasonic sensor initialized");
}
...
float measureDistance() {
  ...
  if (distance == 0 || distance > 400) {
    return 400; // Return max range if invalid
  }
...
void checkObstacles() {
  ...
  if (distance < MIN_DISTANCE && distance > 0) {
    if (!objectDetected) {
      objectDetected = true;
      stopCar();
      sendBTMessage("Obstacle detected at " + String(distance, 1) + " cm. Stopping.");
      Serial.println("Obstacle detected at " + String(distance, 1) + " cm. Stopping.");
    }
  } else {
...
// Modify your existing processCommand function to check for obstacles before moving
void modifiedMoveForward() {
  distance = measureDistance();
  if (distance < MIN_DISTANCE && distance > 0) {
    stopCar();
    sendBTMessage("Cannot move forward. Obstacle at " + String(distance, 1) + " cm");
    return;
  }
  moveForward();
